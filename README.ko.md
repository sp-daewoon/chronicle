...
[skillsmp.com](https://skillsmp.com)은 GitHub를 스크래핑해 공개 skill을 **자동 인덱싱**합니다 — 제출 양식이 없습니다. repo가 공개 상태이고, 표준 `SKILL.md` 형식(chronicle이 따르는)을 쓰며, **⭐ 2개 이상**이면 목록에 나타납니다. chronicle이 유용했다면 ⭐ 하나가 다른 사람들의 발견을 도와줍니다.

## 기여

이슈와 PR을 환영합니다. 각 skill은 독립적이고, 헬퍼는 순수 함수이며 단위 테스트가 있습니다. PR 전에 `npm test`와 `claude plugin validate .`를 실행해 주세요.
